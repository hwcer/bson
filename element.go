...
func (ele *Element) IsEmbedded() bool {
	return ele.t == TypeArray || ele.t == TypeEmbeddedDocument
}
...
func (ele *Element) getOrCreateDoc() Document {
	if ele.t != TypeEmbeddedDocument {
		ele.t = TypeEmbeddedDocument
		ele.v = Document{}
	} else if ele.v == nil {
		ele.v = Document{}
	}
...
func (ele *Element) Reset(t Type, b []byte) error {
	switch t {
	case TypeArray:
